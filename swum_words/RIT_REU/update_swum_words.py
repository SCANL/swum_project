...
class JavaHandler(xml.sax.ContentHandler):
    def __init__(self):
        ...
        # Source: https://stackoverflow.com/questions/45817239/how-can-i-remove-ns-from-xml-in-python?noredirect=1&lq=1
        # LOAD XML AND XSL
        doc = etree.parse('PrimitiveParameters.xml')
        xsl = etree.parse('XSLT_Script.xsl')
        # CONFIGURE AND RUN TRANSFORMER
        transform = etree.XSLT(xsl)    
        self.result = transform(doc)
        ...
        self.idCount = 0
    #def attributeSetter(self,tagRole):
        #for tempTag in self.result.iter(tagRole):
            #if not tempTag.get('id'):
                #tempTag.set('id', str(self.idCount))
    ...
    def XMLWriter(self,locationTagText,currentContent,parameterType,tree, genericStatus):
        ...
        if locationTagText == "variable" or locationTagText == "class" or locationTagText == "constructor" or locationTagText == "interface" or locationTagText == "function":
            idTag = etree.SubElement(swumIdentifierTag, 'id')
            idTag.text = str(self.idCount)
            for tempTag in self.result.iter(locationTagText):
                tempTag.set('id', str(self.idCount))
    ...
    # XML writer for functions, constructor, class, variavle declaration names
    def functionConstructorDeclXMLWriter(self,definedName, role, definedType,genericStatus):
        ...
        if role == "variable" or role == "class" or role == "constructor" or role == "interface" or role == "function":
            idTag = etree.SubElement(swumIdentifierTag, 'id')
            idTag.text = str(self.idCount)
            for tempTag in self.result.findall("name"):
                if tempTag.text == definedName:
                    tempTag.set('id', str(self.idCount))
            self.idCount = self.idCount+1
        locationTag = etree.SubElement(swumIdentifierTag, 'location')
...
if (__name__ == "__main__"):
    ...
    Handler.result.write('temp_new.xml')
